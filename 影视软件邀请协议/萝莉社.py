    ...
    def getInfo(self, url, load_data):
        ...
        try:
            if res.text[0] == "{":
                ...
                data = self.aes.decrypt(str(base64.b64encode(res.content), "utf-8"))
                try:
                    # 如果可以直接编码json格式就返回
...
if __name__ == '__main__':
    # deviceId = "".join([choice("0123456789abcdef") for _ in range(16)])
    ip = "http://adkh.somanylolies.com/loliapi"  # 服务器ip
    # luoli = Luoli(ip, deviceId, "null_null")
    # for i in range(1):
    deviceId = "".join([choice("0123456789abcdef") for _ in range(16)])
    luoli = Luoli(ip, deviceId, "null_null")
    luoli.register()
    luoli.bindParent("A2HEB")
